# How Should Fashion AI Be Evaluated and Held Accountable?

## 학습 목표

벤치마크 점수가 높은 패션 AI도 실제 고객·의복·매장 조건에서는 다르게 작동할 수 있다. 이 장은 오프라인 지표, 실물 검증, 편향과 권리 문제를 하나의 평가·책임 체계로 묶는다.
...
좋은 벤치마크 점수는 첫 질문의 일부에 대한 증거다. 그 점수만으로 나머지 세 질문에 답했다고 해석하면 안 된다. 특히 운영 데이터가 학습·시험 데이터와 달라지는 **분포 변화**가 생기면 벤치마크 점수는 더 이상 현재 성능을 보장하지 않는다.
...
### Python으로 전체 정확도와 집단별 정확도 비교하기

전체 평균만 보면 한 집단에서 반복되는 실패를 놓칠 수 있다. 같은 모델의 결과를 집단별로 나누어 확인한다.

```python
groups = {
    "group_A": {"true": [1, 0, 1], "pred": [1, 0, 1]},
    "group_B": {"true": [1, 0, 1], "pred": [0, 0, 0]},
}

all_results = []
for name, values in groups.items():
    correct = [t == p for t, p in zip(values["true"], values["pred"])]
    all_results.extend(correct)
    print(f"{name} accuracy = {sum(correct) / len(correct):.3f}")

print(f"overall accuracy = {sum(all_results) / len(all_results):.3f}")
```

```output
group_A accuracy = 1.000
group_B accuracy = 0.333
...
- **벤치마크(benchmark)**: 데이터, 분할, 과제, 지표를 정해 모델을 비교하는 규약.
