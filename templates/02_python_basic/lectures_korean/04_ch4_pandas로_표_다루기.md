...
표 형태의 데이터를 반복문으로 한 줄씩 처리하는 것은 느리고 실수가 잦다. 이 장은 pandas DataFrame으로 표 전체를 한 번에 필터·집계·결합하는 기본 동작을 다룬다.
...
`groupby`는 "같은 값을 가진 행끼리 묶어 각 묶음을 하나의 숫자로 요약"한다. SQL의 `GROUP BY`, 「패션 데이터 분석과 AI 활용」의 질문·분석 워크플로우 장에서 배운 집계와 같은 개념이다.
