# Virtual Try-On은 실제 피팅인가

> **이 장의 중심 질문**<br>
> 사람 사진과 상품 사진을 합성한 결과만 보고 실제 사이즈와 착용감까지 판단할 수 있을까?
...
- VITON에서 StableVITON까지 연구 문제의 변화를 비교할 수 있다.
- 시각적으로 자연스러운 착용 이미지와 물리적으로 정확한 피팅을 구분할 수 있다.
- 생성 이미지 평가 지표가 실제 사이즈·압박감·활동성을 보장하지 않는 이유를 말할 수 있다.
...
Diffusion prior는 얼굴과 신체를 그럴듯하게 만드는 데 유리하지만, 강한 생성 능력이 입력 상품에 없는 단추나 주름을 만들어내는 위험도 있다. 따라서 사진의 자연스러움과 상품 충실도를 따로 확인해야 한다.

## 5. 문제 발생: 사진이 자연스러우면 실제로도 맞을까?

아니다. VTON 입력에 신체 치수·봉제 패턴·원단 물성·접촉 조건이 없다면 물리적 피팅을 직접 계산할 수 없다.
