# Is the Trained AI Actually Good?

## 학습 목표

Training Loss가 낮다는 사실은 모델이 연습문제를 잘 푼다는 뜻일 뿐이다. 이 장은 새 데이터에서의 성능(일반화)을 검증 데이터와 지표로 확인하고, 과적합을 어떻게 알아채고 줄이는지 다룬다.
...
### Python으로 threshold에 따른 Metric 비교하기

같은 예측 확률도 임계값을 어디에 두느냐에 따라 최종 예측과 정확도가 달라진다. 따라서 Loss, 확률과 업무 결정 기준을 구분해야 한다.

```python
import numpy as np

y_true = np.array([1, 1, 0, 0])
probability = np.array([0.90, 0.55, 0.60, 0.10])

for threshold in [0.5, 0.7]:
    prediction = (probability >= threshold).astype(int)
    accuracy = np.mean(prediction == y_true)
    print(f"threshold={threshold:.1f}, prediction={prediction}, accuracy={accuracy:.2f}")
```

```output
threshold=0.5, prediction=[1 1 1 0], accuracy=0.75
threshold=0.7, prediction=[1 0 0 0], accuracy=0.75
```

정확도는 같지만 잘못 판단한 사례가 서로 다르다. 반품 위험 탐지처럼 오류 비용이 다른 문제에서는 어떤 사례를 놓쳤는지도 확인한다.
