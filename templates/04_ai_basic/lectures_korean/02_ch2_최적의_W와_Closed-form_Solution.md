# Can We Find the Best W in One Step?

## 학습 목표

단순한 선형 모델에서는 Loss가 가장 작은 $W$를 반복 없이 공식 하나로 바로 계산할 수 있다. 이 공식을 유도해 외우는 것보다, 언제 가능하고 왜 대부분의 AI는 이 방법을 쓰지 않는지 비교하는 것이 이 장의 목적이다.
...
### Python으로 최소제곱해 한 번에 구하기

`np.linalg.lstsq`는 역행렬을 직접 만드는 대신 최소제곱 문제를 안정적인 수치 계산으로 푼다. 첫 값은 절편, 둘째 값은 입력이 한 단위 늘 때의 변화량이다.

```python
import numpy as np

X = np.array([[1, 0], [1, 1], [1, 2]], dtype=float)
y = np.array([52, 65, 78], dtype=float)
