...
정답 $W$를 모를 때 AI는 현재 값에서 Loss가 줄어드는 방향으로 조금씩 움직인다. 이 장은 그 방향을 기울기(gradient)로 구하는 원리와, 보폭·모멘텀 같은 Optimizer 설정이 학습 속도와 안정성을 어떻게 바꾸는지 다룬다.
...
### Python으로 Gradient Descent 다섯 걸음 보기

목표가 $L=(w-3)^2$라면 Gradient는 $2(w-3)$이다. 현재 위치에서 Gradient의 반대 방향으로 이동할 때 `w`가 3에 가까워지고 Loss가 줄어드는지 확인한다.
