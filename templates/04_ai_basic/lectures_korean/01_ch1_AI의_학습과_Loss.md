...
AI가 '학습한다'는 말은 모델 안의 숫자(Parameter)를 데이터에 맞게 조정한다는 뜻이다. 그 조정의 기준이 되는 것이 Loss이며, 이 장은 이후 모든 장이 되풀이하는 "학습 = Loss를 줄이는 $W$ 찾기"라는 한 문장을 가장 단순한 예로 세운다.
...
### Python으로 후보 Parameter의 Loss 비교하기

아래 코드는 세 개의 기울기 후보가 같은 데이터에서 만드는 평균제곱오차를 비교한다. `w=1.3`일 때만 세 관측값을 정확히 지나므로 Loss가 0이 된다.
...
for w in [1.0, 1.3, 1.6]:
    prediction = w * x + 65
    loss = np.mean((y - prediction) ** 2)
    print(f"w={w:.1f}, prediction={prediction}, loss={loss:.1f}")
```

```output
w=1.0, prediction=[55. 65. 75.], loss=6.0
w=1.3, prediction=[52. 65. 78.], loss=0.0
w=1.6, prediction=[49. 65. 81.], loss=6.0
```
