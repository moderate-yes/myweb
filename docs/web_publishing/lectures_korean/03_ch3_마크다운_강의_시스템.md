# Flask 강의 사이트 3: Markdown 콘텐츠와 안전한 경로

## 학습 목표

- 파일명 규칙으로 강의 순서를 일관되게 만든다.
- Markdown을 HTML로 변환해 Template에 전달한다.
- 허용 목록과 경로 검증으로 임의 파일 읽기를 막는다.
...
1. 사용자가 보낸 파일 경로를 그대로 열면 어떤 문제가 생기는가?
2. Markdown에서 만들어진 HTML을 신뢰할 수 없는 입력과 함께 사용할 때 무엇을 검토해야 하는가?
