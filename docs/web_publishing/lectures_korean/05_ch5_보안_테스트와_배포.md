# Flask 강의 사이트 5: 보안·테스트·배포 준비

## 학습 목표

- Secret을 코드와 저장소에서 분리한다.
- 공개·보호 Route의 기대 응답을 자동 테스트한다.
- Debug mode, HTTPS, Cookie와 오류 로그를 배포 전에 점검한다.
...
1. 비밀번호를 Session에 그대로 저장하면 안 되는 이유는 무엇인가?
2. `debug=True`를 공개 서버에서 끄는 이유는 무엇인가?
3. 배포 성공과 서비스 운영 성공은 어떻게 다른가?
